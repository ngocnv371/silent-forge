...
export function reforge(item: Item): Item {
  ...
  const maxPrefixes = Math.min(getRandomNumberOfAffixes(rarity), availablePrefixes.length);
  ...
  const maxSuffixes = Math.min(getRandomNumberOfAffixes(rarity), availableSuffixes.length);
