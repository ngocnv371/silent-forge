...
export function calculateModifiers(items: Item[]): ModifiersMap {
  ...
  console.time('calculateModifiers');
  ...
  for (let i = 0; i < validItems.length; i++) {
    ...
    item.modifiers.forEach((k) => {
      const oldValue = modifiers[k.effect] | 0;
      console.log(`${k.effect}: ${oldValue} + ${k.magnitude}`);
      modifiers[k.effect] = oldValue + k.magnitude;
    });
    console.groupEnd();
  }

  console.groupEnd();
  console.timeEnd('calculateModifiers');
  return modifiers;
}
